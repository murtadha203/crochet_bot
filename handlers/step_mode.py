...
async def jump_to_step_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Prompt user for step number to jump to"""
    query = update.callback_query
    await query.answer()
    
    await query.message.reply_text(
        "🔢 **انتقال السريع**\n\n"
        "لاي خطوة تنتقلين؟ اكتبي الرقم:",
        parse_mode='Markdown'
    )
    return 1  # State for waiting for number


async def jump_to_step_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle step number input"""
    try:
        target_step = int(update.message.text.strip())
        session_id = context.user_data.get('session_id')
        step_gen = context.user_data.get('step_generator')
        
        if not step_gen or not session_id:
            await update.message.reply_text(config.ERROR_MESSAGES['no_session'])
        ...
        if target_step < 1 or target_step > total_steps:
            await update.message.reply_text(
                f"❌ الرقم غلط. الخطوات من 1 الى {total_steps}"
            )
            return 1  # Stay in state
            
        # Update session
        session_mgr.set_current_step(session_id, target_step)
        
        # Show the target step
        # Note: We can't edit the user's text message, so we reply with a new photo message
        await _show_step(update.message, context, target_step)
        
        return ConversationHandler.END
        
    except ValueError:
        await update.message.reply_text("❌ اكتبي رقم صحيح")
        return 1  # Stay in state
